...
type GameStatus = "ANSWER" | "QUESTION" | "WAITINGANSWER" | "WAITINGQUESTION" | "SELECTANSWER" | "BESTANSWER";
...
export function Buddies({ roomCode, users, onExit }: BuddiesProps) {
  ...
  useEffect(() => {
    ...
    const receiveTheBestAnswer = (data: { user: string; answer: string }, status: GameStatus) => {
      setBestAnswer(data);
  ...
  return (
    <div className="buddies" ref={scope}>
      {gameStatus === "QUESTION" && !writtenQuestion && <Question roomCode={roomCode} users={users} onClick={isQuestionWritten} />}
      ...
      {gameStatus === "ANSWER" && !writtenAnswer && socket.id !== question.author && <Answer roomCode={roomCode} users={users} onClick={isAnswerWritten} question={question} />}
      {gameStatus === "ANSWER" && writtenAnswer && socket.id !== question.author && (
        <>
      ...
      {gameStatus === "SELECTANSWER" && <AnswersSelect roomCode={roomCode} users={users} question={question} />}
