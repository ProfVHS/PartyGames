...
server.listen(3000, async () => {
  db.serialize(() => {
    ...
    db.run(
      'CREATE TABLE rooms ("id" INTEGER NOT NULL PRIMARY KEY, "turn" INTEGER NOT NULL, "ready" INTEGER NOT NULL);'
    );
    db.run(
  ...
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM rooms WHERE id = ${room}`, [], (err, turn_row) => {
      if (err) {
        ...
        db.all(
          `SELECT * FROM users WHERE id_room = ${room} AND alive = true`,
          [],
          (err, users_rows) => {
            if (err) {
              reject(err);
...
const Ctb = require("./click-the-bomb")(
  io,
  db,
  usersData,
  updateDataBomb,
