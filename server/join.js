exports = module.exports = function(io, db){
    io.sockets.on('connection', function (socket) {
      ...
      socket.on("create-room", async (data) => {
        ...
        db.run(`INSERT INTO users (id,username,score,alive,id_room) VALUES ("${socket.id}", "${data.name}", 0, true, ${data.randomRoomCode})`);
      ...
      socket.on("join-room", async (data) => {
        ...
        db.run(`INSERT INTO users (id,username,score,alive,id_room) VALUES ("${socket.id}", "${data.name}", 0, true, ${data.roomCode})`);
